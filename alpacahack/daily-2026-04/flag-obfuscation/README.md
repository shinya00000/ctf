...
### ファイル構成
配布ファイルには難読化処理を行う `obfuscator.c` と難読化されたデータが格納された `data.h` が含まれています．
...
さらに復元後のバイナリはフラグを8バイトずつスタックに積む手法（Stack Strings）で保持していますが，平文のまま分割されているだけであるため，静的解析ツールや `strings` コマンドで容易に抽出可能です．
...
出力されたスタック文字列の断片からアセンブリの機械語由来の文字（`H` や `D$` など）を除外し，順番に結合するとFLAGが得られます．